      ...
      <section className="relative flex min-h-[50vh] items-end overflow-hidden px-6 pb-16 pt-32 md:px-12 md:pb-20 lg:px-20">
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl">
          ...
          <p className="mt-6 max-w-xl text-base leading-relaxed text-white/50 md:text-lg">
            Past nights and future dates. Tap a recap to read the full
            write-up, or expand an upcoming slot for details.
          </p>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-10 grid gap-px bg-white/10 md:grid-cols-2">
            {previousEvents.map((event) => (
              <Link
                key={event.slug}
                href={`/test/events/${event.slug}`}
                className="group relative flex flex-col justify-between bg-black p-8 transition hover:bg-white/[0.03] md:p-10"
              >
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-white/30">
                    {event.date}
                  </p>
                  <h3 className="mt-3 font-display text-lg font-bold uppercase tracking-[0.04em] text-white/90 transition group-hover:text-white md:text-xl">
                    {event.title}
                  </h3>
                  <p className="mt-1 text-[11px] uppercase tracking-[0.2em] text-white/35">
                    {event.venue}
                  </p>
                  <p className="mt-4 text-sm leading-relaxed text-white/50 transition group-hover:text-white/65">
                    {event.excerpt}
                  </p>
                </div>
