...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${event.title} | LUDZY Events (Legacy)`,
    description: event.excerpt,
  };
}

export default async function LegacyPreviousEventPage({ params }: Props) {
  const { slug } = await params;
  const event = getPreviousEvent(slug);
  if (!event) notFound();
        ...
        <article className="mt-10">
          ...
          <div className="mt-10 space-y-6 border-t border-white/10 pt-10">
            {event.body.map((paragraph, i) => (
              <p key={i} className="text-base leading-relaxed text-white/65">{paragraph}</p>
            ))}
          </div>
        </article>
      </div>
